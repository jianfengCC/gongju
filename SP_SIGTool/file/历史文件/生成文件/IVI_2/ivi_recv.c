...
void setSignal_SWHC_400(uint8_t *sigbuff, uint8_t len);
    ...
    while(startpos + 3 < msg.len) {
        ...
        if (IVI_SIG_PUB == type) {
            ...
            case 0x500:
                ...
                break;
            case 0x400:
                setSignal_SWHC_400(sigbuff, len - 1);
                break;
            default:
                break;
            }
...
{
    uint8_t bResult = RTE_SIG_OK;
    if (p_SWHC_400_st.data_len - 1 != len) {
        SystemApiLogPrintf(LOG_ERROR_OUTPUT,"sig len error:%d\r\n", len);
        return;
    }
    memcpy(((uint8_t*)&p_SWHC_400_st) + 4, sigbuff, len);

    bResult |= Rte_Write_rpSR_CANMSG_SWHC_400_ComIn_VeCAN_SWHC_SterringWheelHeatSts(&p_SWHC_400_st.SWHC_SterringWheelHeatSts);
    bResult |= Rte_Write_rpSR_CANMSG_SWHC_400_ComIn_VeCAN_SWHC_SterringWheelHeatFaultReason(&p_SWHC_400_st.SWHC_SterringWheelHeatFaultReason);

    if (bResult != RTE_SIG_OK) {
        SystemApiLogPrintf(LOG_ERROR_OUTPUT,"setSignal_SWHC_400 error bResult:0x%x\r\n", bResult);
    }
}
